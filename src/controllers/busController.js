...
async function buscarInformacoes(req, res) {
    try {
        ...
        if (!linhaInfo) return res.status(404).json({ erro: "Nenhum veículo disponível." });

        const veiculosNoSentido = linhaInfo.vs.filter((v) => linhaInfo.sl == sentido);
        if (veiculosNoSentido.length === 0) return res.status(404).json({ erro: "Nenhum veículo no sentido informado." });

        const veiculoMaisProximo = veiculosNoSentido.reduce((maisProximo, veiculoAtual) => {
            const distMaisProx = Math.hypot(maisProximo.py - paradaMaisProxima.py, maisProximo.px - paradaMaisProxima.px);
            const distAtual = Math.hypot(veiculoAtual.py - paradaMaisProxima.py, veiculoAtual.px - paradaMaisProxima.px);
